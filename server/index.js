...
async function ensureBotEnv() {
  ...
  try {
    ...
    // Check extra para la carpeta bot_1 si no existe
    const bot1Dir = path.join(__dirname, '../bot_1');
    if (!fsSync.existsSync(bot1Dir)) {
      console.log('⚠️ Carpeta bot_1 no existe, será creada por el generador o es volumen nuevo.');
    }
...
app.post('/api/bot/:instanceId/start', async (req, res) => {
  ...
  try {
    ...
    exec(`pm2 start ${botScript} --name ${pm2Name} --watch=false --update-env`, {
      ...
      // Verificar después de 15 segundos si el bot conectó
      setTimeout(() => {
        const status = botStatuses.get(instanceId);
    ...
    console.error(`❌ [BOT-START] Error general:`, error);
    res.status(500).json({ success: false, error: error.message });
  }
});
