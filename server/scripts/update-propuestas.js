...
// Nuevas propuestas FEBRERO 2025
const PROPUESTAS_FEBRERO_2025 = [
...
async function updatePropuestas() {
    try {
        ...
        if (existing) {
            ...
            existing.variants = PROPUESTAS_FEBRERO_2025.map(content => ({
                content,
                isActive: true
            }));

            await existing.save();
            console.log(`✅ Template actualizado con ${PROPUESTAS_FEBRERO_2025.length} nuevas variantes`);
        } else {
            console.log('📝 Template no existe, creando nuevo...');
            await TemplateVariant.create({
                category: 'propuestas',
                variants: PROPUESTAS_FEBRERO_2025.map(content => ({
                    content,
                    isActive: true
                }))
            });
            console.log(`✅ Template creado con ${PROPUESTAS_FEBRERO_2025.length} variantes`);
        }
